...
OpenClaw 是快速演進中的開源專案。「快速演進」的另一面是 **CVE 也來得很快**。

最近一次密集事件：**單週爆出九個 CVE，其中一個 CVSS 9.9（接近滿分 10）**。這不是孤例，過去半年類似的密集修補出現過好幾次。
...
把這條指令排進你的週間 routine — 我們建議每週一早上跑一次：

```bash
# 寫進 crontab，每週一 09:00 自動執行
0 9 * * 1 openclaw update && openclaw gateway restart
```

或在 macOS 用 launchd / Linux 用 systemd timer 安排。**不要靠記性**，這種事忘一次就可能讓你的 Agent 暴露在已公開的漏洞中好幾週。

### 訂閱 CVE 通知

至少做一件事：**訂閱 OpenClaw 的 GitHub Security Advisories**（在 repo 頁面 → Security → Advisories → Watch）。新 CVE 公布時你會收到 email，配合每週更新流程能把暴露窗口壓到最小。

<Callout type="warn">
**舊版本不會自動下架。** 即使新版發布了，你跑的還是舊版 — 漏洞依然存在。`openclaw update` 不會自己跑，這是你的責任。
</Callout>
