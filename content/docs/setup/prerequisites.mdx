...
<Callout type="warn">
**macOS 使用者注意**：你的帳號必須是**管理員（Administrator）**。Homebrew 安裝需要 sudo 權限，非管理員帳號會直接失敗。確認方式：**系統設定 → 使用者與群組**，你的帳號旁邊應該標示「管理者」。
</Callout>
...
### 完整支援的模型清單（2026 Q2 更新）

模型景觀在過去兩個月變動很大。下表是目前主流選擇 — 不要照搜尋引擎排前面的舊文章設定，那些有一半已經過時。

| Provider | 旗艦模型 | 性價比模型 | 適合場景 |
|----------|----------|------------|----------|
| **Anthropic** | Opus 4.6 | Sonnet 4.6 | Agent 任務首選；Opus 用於設定和複雜推理，Sonnet 處理日常 |
| **OpenAI** | GPT-5.4 | GPT-5.4-mini | 工具呼叫穩、全能型；Anthropic 切斷第三方訂閱後一週內大量使用者轉移到此 |
| **Google** | Gemini 2.5 Pro | Gemini Flash | 長上下文、大量文件處理；免費額度寬鬆，預算緊時的好起點 |
| **DeepSeek** | DeepSeek R1 | DeepSeek V3 | 中國團隊的強推理模型，價格極具競爭力，Opus 之外處理複雜任務的便宜替代 |
| **Ollama（本地）** | Gemma 3、Llama 4 | — | 完全免費、離線運作；非前沿品質但日常 Agent 任務夠用 |

<Callout type="info">
**我們的推薦：Opus 4.6 當主力**。它是目前 Agent 工作的最強模型。新 API 客戶預設 rate limit 偏低，**一次儲值 $250 美元**可以快速解鎖較高額度，避免設定階段一直撞牆；預算緊的話 $50 起步也能跑。
</Callout>

<Callout type="warn">
**orchestrator role 別用 GPT-5.4-mini**：實測會把 thinking tokens 漏到對話輸出。如果你要做多 Agent 編排，Gemini Flash Lite 是更穩的選擇。
</Callout>

<Callout type="info">
你可以同時設定多把 API Key，讓不同的 Agent（或同一 Agent 的不同任務類型）使用不同的模型。這就是後面[平價模型策略](/docs/cost/budget-models)的核心。
</Callout>
