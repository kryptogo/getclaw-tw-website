...
const agents = [
  {
    role: "CEO Agent",
    Icon: IconCEO,
    desc: "Email triage + 行事曆管理 + 會議摘要",
    tasks: ["重要信件篩選與摘要", "智慧排程與衝突偵測", "會議 briefing 自動生成"],
    delay: 0,
  },
  {
    role: "CFO Agent",
      ...
      <div className="max-w-[1200px] mx-auto">
        <ScrollReveal className="text-center mb-4">
          ...
          <p className="text-text-secondary text-[clamp(15px,1.4vw,18px)] leading-[1.8] max-w-[700px] mx-auto font-light">
            1 Executive Agent = 1 個 AI 助理實例。每個 Agent 綁定一個主要身份，
        ...
        <div className="grid grid-cols-4 gap-6 mt-16 max-lg:grid-cols-2 max-md:grid-cols-1">
          {agents.map((agent) => (
            <ScrollReveal key={agent.role} delay={agent.delay}>
              <div className="bg-bg rounded-2xl p-8 h-full border border-border transition-all hover:shadow-[0_8px_30px_rgba(0,0,0,0.06)] hover:-translate-y-1">
                ...
                <ul className="list-none flex flex-col gap-1.5">
                  {agent.tasks.map((task) => (
                    <li key={task} className="text-sm text-text-muted flex items-start gap-2">
