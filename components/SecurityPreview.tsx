...
const features = [
  { Icon: IconServer, label: "Self-hosted", desc: "資料不上雲" },
  { Icon: IconShield, label: "安全加固", desc: "官方 hardening guide" },
    ...
    <section className="py-32 px-10 bg-bg-dark text-white max-md:py-24 max-md:px-6 max-[480px]:py-16 max-[480px]:px-4" id="security">
      <div className="max-w-[1200px] mx-auto">
        ...
        <div className="grid grid-cols-3 gap-6 max-lg:grid-cols-2 max-md:grid-cols-1">
          {features.map((f, i) => (
            <ScrollReveal key={f.label} delay={i * 0.05}>
              <div className="bg-white/6 backdrop-blur-sm rounded-xl p-6 border border-white/8 h-full">
        ...
        <ScrollReveal className="text-center mt-10">
          <Link
            href="/security"
            className="inline-block text-primary-light font-bold text-sm hover:underline"
          >
            了解完整安全架構 →
          </Link>
        </ScrollReveal>
